...
const BiographyPage = () => {
  ...
  const PositionCard = ({ position, index }) => {
    const getTypeColor = (type) => {
      switch(type) {
        case 'ministerial': return 'bg-red-700';
        case 'parliamentary': return 'bg-red-800';
            ...
            <div className="grid gap-4">
              {position.president && (
                <div className="flex items-center gap-3">
                  <span className="text-sm font-medium text-red-700 w-24">President:</span>
                  <span className="text-red-800">{position.president}</span>
                </div>
              )}
              
              {position.primeMinister && (
                <div className="flex items-center gap-3">
                  <span className="text-sm font-medium text-red-700 w-24">PM:</span>
                  <span className="text-red-800">{position.primeMinister}</span>
                </div>
              )}
              ...
              {position.constituency && (
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-red-800">{position.constituency}</span>
                </div>
              )}
              
              {position.preceded && (
                <div className="flex items-center gap-3">
                  <span className="text-sm font-medium text-red-700 w-24">Preceded:</span>
                  <span className="text-red-800">{position.preceded}</span>
                </div>
              )}
              
              {position.succeeded && (
                <div className="flex items-center gap-3">
                  <span className="text-sm font-medium text-red-700 w-24">Succeeded:</span>
                  <span className="text-red-800">{position.succeeded}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="relative w-full max-w-7xl mx-auto px-6 lg:px-8">
          <div className={`text-center transform transition-all duration-1000 ${
            ...
            <div className="inline-flex items-center bg-white border border-red-200 px-4 py-2 rounded-sm mb-8">
              <User className="w-4 h-4 text-red-800 mr-2" />
              <span className="text-red-800 font-medium text-sm tracking-wide">POLITICAL BIOGRAPHY</span>
            </div>
            ...
            <div className="w-40 h-40 bg-gradient-to-br from-red-50 to-red-100 rounded-full mx-auto mb-8 flex items-center justify-center border-4 border-red-800">
              <img className='rounded-full object-cover object-center w-full h-full' src={HEADER} alt="" />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-4xl mx-auto mb-12">
              <div className="bg-white border border-red-200 p-4 text-center">
                <div className="text-sm text-red-800 font-medium tracking-wider uppercase mb-2">Born</div>
                <div className="text-red-900">{personalDetails.born}</div>
              </div>
              <div className="bg-white border border-red-200 p-4 text-center">
                <div className="text-sm text-red-800 font-medium tracking-wider uppercase mb-2">Birthplace</div>
                <div className="text-red-900">{personalDetails.birthplace}</div>
              </div>
              <div className="bg-white border border-red-200 p-4 text-center">
                <div className="text-sm text-red-800 font-medium tracking-wider uppercase mb-2">Residence</div>
                <div className="text-red-900">{personalDetails.residence}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-2xl mx-auto">
              <div className="text-center">
                ...
                <div className="text-sm text-red-600 font-medium tracking-wider uppercase">Ministerial Positions</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-light text-red-800 mb-2">15+</div>
                <div className="text-sm text-red-600 font-medium tracking-wider uppercase">Years in Politics</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-light text-red-800 mb-2">Current</div>
                <div className="text-sm text-red-600 font-medium tracking-wider uppercase">MP Status</div>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className={`transform transition-all duration-1000 delay-300 ${
              ...
              <div className="space-y-4">
                {politicalAffiliations.map((affiliation, index) => (
                  <div key={index} className="border-l-4 border-red-300 pl-6 pb-4">
                    <h4 className="font-medium text-red-900 mb-1">{affiliation.party}</h4>
        ...
        <div className="w-full max-w-7xl mx-auto px-6 lg:px-8">
          <div className={`text-center transform transition-all duration-1000 delay-600 ${
            ...
            <h3 className="text-2xl lg:text-3xl font-light text-white mb-6">
              Learn More About
              <span className="font-normal text-red-200 ml-2">His Work</span>
            </h3>
            
            <div className="w-16 h-0.5 bg-red-400 mx-auto mb-8"></div>
            
            <p className="text-red-100 leading-relaxed text-lg mb-8 max-w-2xl mx-auto">
              Stay updated with his ongoing political initiatives and contributions to Nepalese society.
            </p>
            
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button className="bg-white text-red-800 hover:bg-white px-8 py-3 font-medium tracking-wide transition-all duration-200 shadow-sm hover:shadow-md">
                VIEW RECENT WORK
              </button>
              <button className="border border-red-400 text-red-200 hover:bg-red-900 px-8 py-3 font-medium tracking-wide transition-all duration-200">
                CONTACT OFFICE
              </button>
            </div>
          </div>
